...
function showMainWork(){passwordPanel.hidden=true;let k=document.querySelector('.admin-kpis');if(k)k.hidden=true;let w=adminShell();if(w)w.scrollIntoView({behavior:'smooth',block:'start'})} function bindAdminTools(){let buttons=[...document.querySelectorAll('.admin-side nav button')];if(buttons.length<7)return;buttons[0].onclick=()=>location.reload();buttons[1].onclick=()=>{showMainWork();openProducts()};buttons[2].onclick=()=>{showMainWork();openFragrances()};buttons[3].onclick=()=>{showMainWork();openVariants()};buttons[4].onclick=()=>{showMainWork();openPhotos()};buttons[5].onclick=()=>{showMainWork();openOrders()};let cards=[...document.querySelectorAll('.admin-actions button')];if(cards[0])cards[0].onclick=()=>{showMainWork();openProducts()};if(cards[1])cards[1].onclick=()=>{showMainWork();openFragrances()};if(cards[2])cards[2].onclick=()=>{showMainWork();openVariants()}}
